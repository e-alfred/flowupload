			...
			<ul id="locations" class="with-icon">
				<AppNavigationItem active="true"
					...
					<template slot="actions">
						<ActionButton icon="icon-edit" v-on:click="openLocationInFiles(location.path)">
							{{ t('flowupload', 'Open') }}
						</ActionButton>
						<ActionButton icon="icon-starred" v-if="!location.starred" v-on:click="starLocation(location.path)">
							{{ t('flowupload', 'Star') }}
						</ActionButton>
						<ActionButton icon="icon-starred" v-if="location.starred" v-on:click="unstarLocationByPath(location.path)">
							{{ t('flowupload', 'Unstar') }}
						</ActionButton>
						<ActionButton icon="icon-delete" v-on:click="removeLocation(location.path)">
							{{ t('flowupload', 'Remove') }}
						</ActionButton>
					</template>
				</AppNavigationItem>
			...
			<div v-activeLocationFileDropZone style="padding: 2.5%; width:auto" v-if="!loading">
				...
				<div id="locationSelected" ng-cloak v-if="activeLocation != undefined">
					...
					<div class="buttonGroup">
						...
						<input id="FolderSelectInput" type="file" multiple="multiple" webkitdirectory="webkitdirectory" v-on:change="filesSelected">
					</div>
					<hr>
					<div class="buttonGroup">
					...
					<table id="uploadsTable">
						<thead>
							<tr>
								...
								<th v-on:click="selectSortingMethod('name')">
									<a class="noselect">
									<span>{{ t('flowupload', 'Name') }}</span>
									<span v-bind:class="{'icon-triangle-n':  (sort == 'name' && sortReverse), 'icon-triangle-s': (sort == 'name' && !sortReverse)}" class="sortIndicator"></span>
									</a>
								</th>
								<th></th>
								...
								<th v-on:click="selectSortingMethod('size')" style="width:10%">
									<a class="noselect">
									<span>{{ t('flowupload', 'Size') }}</span>
									<span v-bind:class="{'icon-triangle-n':  (sort == 'size' && sortReverse), 'icon-triangle-s': (sort == 'size' && !sortReverse)}" class="sortIndicator"></span>
									</a>
								</th>
								<th v-on:click="selectSortingMethod('progress')" style="width:20%">
									<a class="noselect">
									<span>{{ t('flowupload', 'Progress') }}</span>
									<span v-bind:class="{'icon-triangle-n':  (sort == 'progress' && sortReverse), 'icon-triangle-s': (sort == 'progress' && !sortReverse)}" class="sortIndicator"></span>
									</a>
								</th>
							</tr>
						</thead>
